...
<?php $this->_view->headScript()->captureEnd();
			$tagHtml = sprintf('<a href="%s" %s id="tag%s" rel="amf:tag"><span>Dieses Rezept wurde %s mal mit dem Schlagwort </span><span property="amf:tag">%s</span><span> versehen.</span></a>', htmlSpecialChars($tag->getParam('url')), $attribute,str_replace(array('-',' ','.'),array('_','__','___'),$tag->getTitle()), $tag->getWeight(), $tag->getTitle());
